...
import { ArrowRight, CheckCircle2, Phone, ArrowDown } from "lucide-react";

import { processDetailed } from "@/components/landing/data";
...
export default function ProcessPage() {
  return (
      <main>
          ...
          <div className="container-wide flex flex-col items-center gap-5 text-center">
            ...
            <Reveal delay={0.1}>
              <p className="max-w-xl text-base md:text-lg leading-7 md:leading-8 text-white/80">
                Lucrăm structurat, cu pași clari, astfel încât strategia să se transforme rapid în acțiuni care livrează rezultate. De la analiză la implementare — fiecare etapă contează.
              </p>
            </Reveal>
        ...
        <section className="section-space border-b border-border/70 bg-background">
          <div className="container-wide">
            <Reveal>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {processDetailed.map((step, index) => {
                  const Icon = step.icon;
                  return (
                    <Link
                      key={step.slug}
                      href={`#${step.slug}`}
                      className="group flex flex-col items-center gap-3 rounded-2xl border-[2px] border-foreground/90 bg-card p-6 text-center shadow-[3px_3px_0px_0px_var(--foreground)] transition-all hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[2px_2px_0px_0px_var(--foreground)]"
                    >
                      <div className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-accent/10 border border-accent/20 transition-colors group-hover:bg-accent/20">
                        <Icon className="h-6 w-6 text-accent" strokeWidth={2.5} />
                      </div>
                      <Badge variant="accent" className="text-xs">{`Pasul 0${index + 1}`}</Badge>
                      <h3 className="text-base font-semibold text-foreground">{step.title}</h3>
                      <p className="text-sm leading-6 text-foreground/60">{step.subtitle}</p>
                      <ArrowDown className="h-4 w-4 text-foreground/30 transition-transform group-hover:translate-y-1 group-hover:text-accent" />
                    </Link>
                  );
                })}
              </div>
        ...
        {/* Individual Process Step Sections */}
        <div id="pasi">
          {processDetailed.map((step, index) => {
                    ...
                    <div className="space-y-6">
                      <Reveal>
                        <div className="flex items-center gap-3">
                          ...
                          <Badge variant="accent">{`Pasul 0${index + 1}`}</Badge>
                          <span className="text-sm font-medium text-foreground/50">· {step.subtitle}</span>
                        </div>
                      </Reveal>
                  ...
                  <Reveal delay={0.1}>
                    <div className="rounded-2xl border-[3px] border-foreground/90 bg-primary p-6 md:p-8 shadow-[6px_6px_0px_0px_var(--foreground)]">
                      <h3 className="text-lg font-semibold text-primary-foreground mb-4">
                        Ce primești la finalul acestei etape
                      </h3>
            ...
            <div className="rounded-2xl border-[3px] border-foreground/90 bg-primary shadow-[6px_6px_0px_0px_var(--foreground)] text-primary-foreground p-8 md:p-12 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <h2 className="mt-4 max-w-3xl text-3xl font-bold leading-tight md:text-4xl text-primary-foreground">
                  Ești pregătit să lucrăm împreună?
                </h2>
                <p className="mt-4 max-w-2xl text-base font-medium leading-7 text-primary-foreground/80">
                  Contactează-ne și vom trece prin fiecare etapă alături de tine — de la strategie la rezultate vizibile.
                </p>
